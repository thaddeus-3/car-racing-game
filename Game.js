class Game{
    ...
    play() {
        ...
        textSize(20)
        fill("black")
        text("CONGRATULATIONS",50,0)
        for(var plr in allPlayers){
            if(allPlayers[plr].rank===1){
                text("1st:"+allPlayers[plr].name,0,80)
                image(goldimg,200,80,80,80)
            }
            if(allPlayers[plr].rank===2){
                text("2nd:"+allPlayers[plr].name,0,150)
                image(silverimg,200,150,80,80)
            }
            if(allPlayers[plr].rank===3){
                text("3rd:"+allPlayers[plr].name,0,220)
                image(bronzeimg,200,220,80,80)
            }
            if(allPlayers[plr].rank===4){
                text("4th:"+allPlayers[plr].name,0,290)
